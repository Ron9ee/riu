...
const unsigned long DEBUG_MODE_DURATION = 8000;

const int DEBUG_BUTTON_PIN = 27;
unsigned long lastButtonPress = 0;
const unsigned long BUTTON_DEBOUNCE_MS = 300;

const char* SETUP_SSID = "Riu-Setup";
const char* MDNS_NAME = "riu";
...
void setup() {
  Serial.begin(115200);
  Serial.println("Riu Assistant Starting...");
  startupTime = millis();
  ...
  setupDisplay();
  setupServo();
  preferences.begin("riu", false);
  loadWiFiCredentials();
  
  Serial.println("✅ Riu initialized - animations will play while WiFi connects");
}
...
void handleRoot() {
  server.sendHeader("Access-Control-Allow-Origin", "*");
  String html = "<!DOCTYPE html><html><head><title>Riu Assistant</title>";
  ...
  html += "<script>";
  html += "async function resetWiFi(){if(confirm('This will clear WiFi settings and restart Riu. Continue?')){try{await fetch('/api/reset',{method:'POST'});alert('Riu is restarting in setup mode...');}catch(e){alert('Riu is restarting...');}}}";
...
void handleSetupPage() {
  String html = "<!DOCTYPE html><html><head><title>Riu Setup</title>";
  ...
  html += "<button type='submit'>Connect Riu</button>";
  html += "</form>";
  html += "<p style='text-align:center;margin-top:20px;font-size:12px;color:#666;'>";
  html += "Riu will connect to your WiFi and restart.</p>";
  html += "</div></body></html>";
...
void handleWiFiConfig() {
  ...
  html += "<p>Riu is connecting to " + ssid + "</p>";
...
void handleWiFiSettings() {
  ...
  String html = "<!DOCTYPE html><html><head><title>Riu WiFi Settings</title>";
  html += "<style>body{font-family:Arial,sans-serif;max-width:600px;margin:50px auto;padding:20px;}";
  html += "button{background:#007bff;color:white;padding:10px 20px;border:none;border-radius:5px;margin:5px;cursor:pointer;}";
  html += "button.danger{background:#dc3545;}button:hover{opacity:0.9;}</style></head><body>";
  html += "<h1>Riu WiFi Settings</h1>";
  html += "<p><strong>Current Network:</strong> " + WiFi.SSID() + "</p>";
  html += "<p><strong>IP Address:</strong> " + WiFi.localIP().toString() + "</p>";
  html += "<p><strong>Signal Strength:</strong> " + String(WiFi.RSSI()) + " dBm</p>";
  html += "<h3>Actions:</h3>";
  html += "<button onclick=\"if(confirm('Reconfigure WiFi? Riu will restart in setup mode.'))window.location='/wifi?action=reset'\">Change WiFi Network</button>";
  ...
  if (server.arg("action") == "reset") {
    ...
    html += "<p>Riu is restarting in setup mode...</p>";
    html += "<p>Connect to \"Riu-Setup\" to reconfigure.</p></body></html>";
...
void drawDebugInfo() {
  ...
  if (isInSetupMode) {
    ...
    display.drawStr(0, 36, "  Riu-Setup");
...
void drawFocusAnimation() {
  ...
  static unsigned long lastFrameTime = 0;
  static unsigned long lastStart = 0;
  static int lastMilestoneShown = 0; 
  static unsigned long milestoneShowTime = 0;
  const unsigned long MILESTONE_DISPLAY_MS = 3000; 
