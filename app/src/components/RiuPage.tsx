...
import { useRiuSync } from '@/contexts/RiuContext';
...
interface RiuPageProps {
  onPageChange?: (page: 'dashboard' | 'yourriu' | 'tasks' | 'reminders' | 'events' | 'notifications' | 'pomodoro' | 'calendar' | 'activity' | 'timetracking' | 'settings' | 'notes') => void;
  theme?: 'clean' | 'retro';
}

const RiuPage: React.FC<RiuPageProps> = ({ theme = 'clean' }) => {
  const {
    isConnected,
    isConnecting,
    riuStatus,
  ...
  const handleResetWiFi = async () => {
    if (!confirm('This will clear WiFi settings and restart Riu in setup mode. Continue?')) return;
    ...
    try {
      ...
      console.log('Reset sent, Riu is restarting...');
      disconnect();
    }
  ...
  const playAndSendToRiu = () => {
    ...
    if (isConnected) {
      const pattern = servoPatterns[selectedPattern];
      console.log(`📡 Sending ${pattern.animationName} to Riu`);
      ...
      <div className="max-w-2xl mx-auto px-6 py-8">
        ...
        {!isConnected && (
          <Card className={theme === 'retro' ? "border-2 border-black rounded-2xl shadow-[4px_4px_0_0_rgba(0,0,0,0.3)]" : ""}>
            <CardHeader className="pb-4">
              <CardTitle className="text-lg">Connect to Riu</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              {/* Simple instruction */}
              <p className="text-sm text-muted-foreground">
                Press the <strong>BOOT button</strong> on your Riu to see the IP address, then enter it below.
              </p>

              {/* Connection input */}
              <div className="flex gap-2">
                <Input
                  placeholder="riu.local or IP address"
                  value={localIP}
            ...
            <CardContent>
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <div className="font-medium font-mono text-sm">{riuStatus.ip}</div>
                </div>
                <div>
                  <div className="text-xs text-muted-foreground">Uptime</div>
                  <div className="font-medium">{formatUptime(riuStatus.uptime)}</div>
                </div>
                <div>
                  <div className="text-xs text-muted-foreground">Animation</div>
                  <div className="font-medium capitalize">{riuStatus.animation}</div>
                </div>
              </div>

              {riuStatus.task && (
                <div className="mt-4 p-2 bg-muted rounded text-sm">
                  <span className="text-muted-foreground">Task:</span> {riuStatus.task}
                </div>
              )}
        ...
        <Card className={`mt-6 ${theme === 'retro' ? "border-2 border-black rounded-2xl shadow-[4px_4px_0_0_rgba(0,0,0,0.3)]" : ""}`}>
          ...
          <CardContent className="space-y-6">
            ...
            <div className="flex gap-2">
              <Button
                onClick={isPlaying ? stopSimulation : playAndSendToRiu}
              ...
              >
                {isPlaying ? (
                  ...
                  <>
                    ...
                    {isConnected ? 'Play & Send to Riu' : 'Play Preview'}
                  </>
                )}
              </Button>
              <Button
            ...
            <div className={`text-xs text-center py-2 px-3 rounded ${isConnected ? 'bg-green-500/10 text-green-600' : 'bg-muted text-muted-foreground'}`}>
              {isConnected 
                ? `✓ Connected to Riu (${customIP}) - animations will sync` 
